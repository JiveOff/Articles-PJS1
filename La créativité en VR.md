La réalité dans laquelle nous vivons est très limitée physiquement, donnons un exemple concret: il est nécessaire d'acquérir les consommables ou matériaux adéquats pour peindre et nous sommes bien sûr limités par la taille de la toile sur laquelle nous peignons.

Il est toutefois possible d’exprimer sa créativité en utilisant des logiciels sur ordinateur avec son clavier et sa souris (et potentiellement une tablette graphique !) mais nous allons parler ici de **réalité virtuelle**, car oui, il est possible de sculpter, dessiner en trois dimensions ou encore construire de petites maisons en blocs de terre (oui oui !) en réalité virtuelle et sans aucune limite.

L'immersion se fait avec un casque spécial et une manette ayant 6 degrés de liberté pour chaque main pour ainsi pouvoir être immergé dans un monde en **3D** et en **360°** pour ainsi laisser libre cours à son imagination.

Le marché de la création artistique en réalité virtuelle ne cesse de s’agrandir et même chose pour les possibilités. Dans cet article, nous allons traiter de trois célèbres applications qui s'avèrent incontournables dans la création artistique en réalité virtuelle:

<div style="text-align: center;">
    <img src="https://i.jiveoff.fr/GAvph.png" style="width:15%; margin-right: 10%; vertical-align: middle;" />
    <img src="https://i.jiveoff.fr/wKpE8.tilt-brush-logo-02-ps4-23jan19-en-us" style="width:30%; margin-right: 10%; vertical-align: middle;" />
    <img src="https://i.jiveoff.fr/wqWkt.PJQMq1gnq-JWlC-cgTwnhyJJWap01FEKfm_x5O0Ab0zgd_UdgjO5rrpYx3CMUUV01NLZDTnoM22aB41nYoBkyjA7Do4RUBredR-2IaSm4nhVJ0tRrMRRhAmRJWyWXDwFiOnAXtk" style="width:15%; vertical-align: middle;" />
</div>

<div style="text-align: center;">
    <i>Il est possible que vous reconnaissiez un de ces logos! On parle bien ici du célèbre jeu, Minecraft.</i>
</div><br />


Présentons de ce pas la première application.

# <img src="https://i.jiveoff.fr/GAvph.png" style="width:10%; margin-right: 20px; vertical-align: middle; float: right;" />Oculus Medium

**Medium** est une application **référence** dans le secteur de la création en réalité virtuelle. Celle-ci est développée par Oculus jusqu'au 6 décembre 2019 où elle a été rachetée par le géant américain de la création artistique, **Adobe**. Medium est sorti le 5 décembre 2016 et fêtera cette année ses **4 ans**. L'application est totalement gratuite et tous public.

Cette application permet aux utilisateurs de sculpter en utilisant leurs « *mains virtuelles* » (c’est-à-dire leurs manettes) et en 360°. Il est possible de se déplacer autour de sa propre œuvre et d'ainsi peaufiner les moindres détails de sa sculpture.   

Il est alors possible de laisser place à sa propre imagination pour ainsi concevoir de très belles pièces.

<div style="text-align: center; margin-bottom: 35px;">
    <img src="https://i.jiveoff.fr/7h36I.jpg" style="width:50%; margin-bottom: 15px;"><br />
    <i>Des sculptures réalisées par la communauté.</i>
</div>


Il est également possible de modéliser des sculptures assez complexes d'un personnage issu de l'animation japonaise à un paysage extrêmement détaillé.

<div style="text-align: center; margin-bottom: 35px;">
    <iframe style="width: 100%;" height="390" src="https://www.youtube.com/embed/-gYGNQhPI34" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe><i>Démonstration de Oculus Medium</i>
</div>


Il est également possible d'exporter ses créations sur des formats plus conventionnels pour ainsi les utiliser dans des vrais logiciels de modélisation comme *Autodesk Maya* ou un logiciel de texturisations comme *Substance*.



Oculus Medium est spécialement conçu pour l'écosystème **Oculus** mais il est toutefois possible d'utiliser Medium sur d'autres casques supportant **SteamVR** avec le logiciel *Revive* disponible [sur GitHub](https://github.com/LibreVR/Revive).

Vous pouvez télécharger **Oculus Medium** sur ce [lien](https://oculus.com/medium).

Après la sculpture, passons au dessin avec **Google Tilt Brush**.

# <img src="https://i.jiveoff.fr/wKpE8.tilt-brush-logo-02-ps4-23jan19-en-us" style="width:15%; margin-right: 20px; vertical-align: middle; float: right" />Tilt Brush

**Tilt Brush** est une application développée cette fois par le géant **Google**. Elle sort le 5 avril 2016 en même temps que le **HTC Vive**, un des premiers kits de réalité virtuelle. Cependant, cette application est payante et représente ici une alternative payante et plus complète de Oculus Medium. Cette application est elle aussi très facile à utiliser et est destinée à tous public de tout âge.

Comme Medium, vous vous retrouvez dans un monde virtuel où vous pouvez dessiner en **3D**. Votre manette droite vous servira à dessiner et la gauche, à manipuler votre palette, comme un vrai artiste.

<div style="text-align: center;">
    <img src="https://3dscanexpert.com/wp-content/uploads/3d-scan-expert-poly-tiltbrush-tutorial-header-1024x593.jpg" style="width: 75%; margin-bottom: 15px"><br/>
    <i>Outils disponibles sur Tilt Brush.</i>
</div><br />


Il est également possible de rejoindre des amis sur une session Tilt Brush et donc de **collaborer** sur un même projet en temps réel!

<div style="text-align: center;">
    <iframe style="width: 100%;" height="395" src="https://www.youtube.com/embed/5oHykuV9I6I" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
    <i>Démonstration du multijoueur de Tilt Brush.</i>
</div><br />

Tilt Brush est disponible sur **une très grande majorité** de plateforme contrairement à Oculus Medium, celle-ci est disponible sur Steam, Oculus Store, PlayStation VR et bien d'autres plateformes.

Plus d'informations sur le site de Tilt Brush: [https://www.tiltbrush.com/](https://www.tiltbrush.com/)

# <img src="https://i.jiveoff.fr/wqWkt.PJQMq1gnq-JWlC-cgTwnhyJJWap01FEKfm_x5O0Ab0zgd_UdgjO5rrpYx3CMUUV01NLZDTnoM22aB41nYoBkyjA7Do4RUBredR-2IaSm4nhVJ0tRrMRRhAmRJWyWXDwFiOnAXtk" style="width:15%; margin-right: 20px; vertical-align: middle; float: right" />Minecraft

Cette application contraste bien avec les deux autres présentées ci-dessus mais nous ne pouvons pas passer à côté d'un des jeux vidéos du siècle: **Minecraft**. Le nombre d'exemplaires vendus témoigne bien de sa popularité: **plus de 200 millions d'exemplaires**. Il est possible de construire sur ce jeu tout ce qui vous viens à l'esprit à l'aide de blocs de constructions.



<center>
    <img src="https://i.jiveoff.fr/tcBZh.390" style="width: 35%; margin-bottom: 15px"><br/>
    <i>Un vaste choix de blocs de construction.</i>
</center>



Il est possible de construire par exemple:

- des *pixel art*, qui sont des constructions inspirées d'une image et réalisées en 2 dimensions.
- de *l'organique*, qui s'inspire de la réalité et réalisé ici, en 3 dimensions.
- des maisons en terre, digne des débutants de Minecraft.



<center><div style="display: inline-block;width: 49%; margin-bottom: 10px;"><img src="https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/86d613b9-bca5-4cd3-823d-46451b331f53/d4z4wwr-01268d0b-21ce-4c62-b27c-b3609d4c08f2.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJ1cm46YXBwOiIsImlzcyI6InVybjphcHA6Iiwib2JqIjpbW3sicGF0aCI6IlwvZlwvODZkNjEzYjktYmNhNS00Y2QzLTgyM2QtNDY0NTFiMzMxZjUzXC9kNHo0d3dyLTAxMjY4ZDBiLTIxY2UtNGM2Mi1iMjdjLWIzNjA5ZDRjMDhmMi5wbmcifV1dLCJhdWQiOlsidXJuOnNlcnZpY2U6ZmlsZS5kb3dubG9hZCJdfQ.ClKfpS3YcE6AXg1IL4KrsPpytx4qeVT9bTGRY-b-5T0" style="width: 80%; margin-bottom: 10px;"><br /><i>Exemple de Pixel Art</i></div><div style="display: inline-block;width: 49%; margin-bottom: 10px;"><img src="https://i.ytimg.com/vi/7zkHyZ3bF80/maxresdefault.jpg" style="width: 80%; margin-bottom: 10px;"><br /><i>Exemple de build organique</i></div><div style="display: inline-block;width: 49%;"><img src="https://fiverr-res.cloudinary.com/images/q_auto,f_auto/gigs/134758318/original/6092a2588f4127ea47741ade1d12ba852f9bd6ea/build-a-minecraft-dirt-house-for-you.jpg" style="width: 80%; margin-bottom: 10px;"><br /><i>Une maison en terre digne d'un débutant</i></div></center>



Le jeu a connu beaucoup de changements depuis sa sortie de 2009 et propose aujourd'hui un mode de jeu unique qui est bien sûr la **réalité virtuelle**.

La première version de ce mode de jeu avait été crée par la communauté sous forme d'un *mod* appelé **ViveCraft**. Cette modification permet aux joueurs de pouvoir interagir avec le monde avec ses manettes, du mouvement à la préparation de nourriture. Il est possible d'également rejoindre ses amis sur des serveurs comme [Hypixel](https://hypixel.net) et jouer ensemble en réalité virtuelle.

<center>
    <img src="https://i0.wp.com/www.vivecraft.org/wp-content/uploads/2016/12/ezgif.com-839689d05b.gif?w=840" style="width: 55%; margin-bottom: 15px"><br />
    <i>Démonstration de ViveCraft</i>
</center>



Il est d'ailleurs possible de *terminer* le jeu en réalité virtuelle comme le témoigne la vidéo ci-dessous:

<center><iframe style="width: 100%" height="390" src="https://www.youtube.com/embed/5TI96_nqhng" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe></center>



La *deuxième version* de cette modification a été (contrairement à la modification **ViveCraft**) créée par les développeurs de Minecraft mais celle-ci présente un bon nombre d'inconvénients comme l'obligation d'utiliser Windows 10 et de posséder un exemplaire du jeu sur votre compte Microsoft.

Toutefois, il y a eu beaucoup d'efforts pour améliorer le confort des joueurs contre le **motion sickness** (mal des transports en français).
